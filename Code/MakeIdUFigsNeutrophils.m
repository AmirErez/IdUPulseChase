...
%% Import
dirname = '../Data/';
savefile = [dirname 'RawNeutrophils'];
Cols = 'brg';
Markers = 'os^'; 
...
%% Stability analysis
tabStability = readtable('../Data/RawNeutrophils/StabilityNeut.csv');

tabStability = join(expdata.tabSamples,tabStability);

figname = 'NeutStability';
...
%     CoarseGrid = struct;
CoarseGrid.xchan = 'Sm154Di_154Sm_CD11b';
CoarseGrid.xchanName = 'CD11b';
CoarseGrid.ychan = 'Pr141Di_141Pr_Ly_6G';
CoarseGrid.ychanName = 'Ly6G';
CoarseGrid.EdUChannel = 'I127Di_127I_IdU';
    
 
% x direction
warning('Overwriting x slice size');
CoarseGrid.SliceSizex = 0.1;
CoarseGrid.xlim = [1 3];
...
%     if(isempty(EventsInPCA1{yId})), EventsInPCA1{yId} = []; end
%     EventsInTime{tInd} = [EventsInTime{tInd} 10^FullData.CoarseGrid.y(yId)];
%     EventsInPCA1{yId} = [EventsInPCA1{yId} FullData.time(tInd)];
%     
